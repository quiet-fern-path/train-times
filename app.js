...
//
// All three, including the ~15MB schedule: a revalidation is cheap, and
// deliberately isn't throttled by hand here. See the schedule-check note
// below for why the obvious "check the big one less often" is the wrong
// lever.
...
// ── Waiting on a newly added route's schedule ───────────────────────
// Re-checking data/schedule.json often is NOT expensive, which is why
// checkForDataUpdates() above doesn't throttle it: GitHub Pages sends an
// ETag and `Cache-Control: max-age=600`, so inside ten minutes the browser
// answers from its own HTTP cache without touching the network, and after
// that a revalidation is a 304 with an empty body (both measured against the
// real deployment). sw.js no longer re-stores an unchanged response either,
// so the 15MB Cache Storage write that used to happen on every check is gone.
// A hand-rolled cooldown on top of that would only add staleness: the server
// already states its own freshness policy, and honouring that is strictly
// better than guessing a number here.
//
// What *is* needed is the opposite — a case where focus-driven checks aren't
// frequent enough. A route added from the settings page (the builder commits
// routes.json) has no schedule data until the Action commits it: the first 7
// days in ~2-3 minutes, then a days-7-89 backfill. For those minutes the
// route is in ROUTES with an empty list and the reader is sitting there
// watching it — not switching tabs, so nothing triggers a focus check. So
// while that's true, tickMinute() polls as well.
//
// How long a route is allowed to hold that open. The fast phase normally
// lands in 2-3 minutes, so this is generous headroom, and it bounds the case
// where the data is never coming at all: a broken Action, or a genuine
// station pair with no direct service, whose fetch legitimately yields zero
// legs and so never satisfies routeHasScheduleLegs().
const SCHEDULE_EAGER_WINDOW_MS = 15 * 60 * 1000;
// routeId -> when we first noticed it had no schedule data, so the window is
// per route: one route timing out never stops a route added later from
...
// board, so "has no schedule legs" is their permanent, correct state, not a
// wait. Counting one here would poll every minute for as long as the quick
// route existed.
function routesAwaitingSchedule() {
  ...
  for (const route of ROUTES) {
    ...
    if (routeHasScheduleLegs(route.id)) {
      delete scheduleWaitStartedAt[route.id]; // arrived — release the window
      continue;
    }
...
// Only fires while a route is actually waiting — the routine case is already
// covered by checkForDataUpdates() above.
function checkForScheduleUpdate() {
  if (!routesAwaitingSchedule().length) return;
